...
mod tests {
    ...
    fn read_text_page_errors() {
        let gone = std::env::temp_dir().join("dsh-preview-face-gone-nope");
    ...
    fn read_all_bytes_cap() {
        ...
        cleanup(&path);
        let gone = std::env::temp_dir().join("dsh-preview-face-all-gone");
    ...
    fn stat_version_changes_with_content() {
        ...
        assert_ne!(v1, v2);
        cleanup(&path);
        let gone = std::env::temp_dir().join("dsh-preview-face-stat-gone");
