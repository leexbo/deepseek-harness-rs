...
async fn manual_compact_forces_summary_and_replays_without_recall() {
    use dsh_agent_loop::FoldOutcome;
    ...
    // 落档载荷:summary + 统计(UI 标记行素材)
    {
        ...
        let visible = dsh_session::derive_visible_messages(l.iter());
...
/// 直接拒)。锁定 `dsh_compaction::CompactRange::prefix_len` 的切片语义。
#[tokio::test]
async fn second_compaction_prefix_covers_through_seq_message() {
    ...
    let expected = {
        ...
        dsh_session::message_from_event(&ev.r#type, &ev.data).expect("消息面")
    };
    assert_eq!(
        prefix.last(),
        Some(&expected),
