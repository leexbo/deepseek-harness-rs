...
use liuma_session::{Attribution, EventEnvelope, EventLog, attribution_chain};
use serde_json::{Value, json};

/// 固定输出工具(E5 只断言归因链,不关心真实 bash;自持实现使
/// host-core 测试不依赖 liuma-tools,消除 dev 依赖环)
struct EchoTool;
...
async fn audit_attributable_and_replayable() {
    let dir = std::env::temp_dir().join(format!("liuma-audit-{}", std::process::id()));
...
fn tampered_attribution_refused_on_rebuild() {
    ...
    let err = liuma_session::decode_envelope(&raw).expect_err("必须拒绝");
