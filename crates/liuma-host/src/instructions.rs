...
//!
//! 三层职责(全宿主侧 IO + 纯函数):
//! - **发现**:全局 `<LIUMA_HOME|~/.liuma>/AGENTS.md` + 从 workspace 向上找
...
pub struct DiscoveredFile {
    ...
    /// 显示路径(全局 = `~/.liuma/AGENTS.md`;项目内 = 相对项目根)
    pub display: String,
...
/// liuma 根(env `LIUMA_HOME` 覆盖;默认 `~/.liuma`)。registry 会话根与
/// 全局指令文件共用同一解析,避免两处约定漂移。
pub fn default_liuma_root() -> PathBuf {
...
impl InstructionRuntimeState {
    ...
    fn visible_map(
        log: &[liuma_session::EventEnvelope],
    ) -> (
    ...
    pub fn restore_from_log(&mut self, log: &[liuma_session::EventEnvelope]) {
...
mod tests {
    ...
    fn temp_dir(tag: &str) -> PathBuf {
        let dir = std::env::temp_dir().join(format!("liuma-instr-{tag}-{}", std::process::id()));
    ...
    fn renders_wrapped_sections_with_budget_ordering() {
        ...
        let files = vec![
            mk("~/.liuma/AGENTS.md", "ug", "user-global", "global rules"),
        ...
        let gpos = r
            .text
            .find("Instructions from: ~/.liuma/AGENTS.md")
            .unwrap();
