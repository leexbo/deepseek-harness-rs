...
use liuma_session::envelope::EnvelopeError;
use thiserror::Error;
...
impl LogBackend {
    /// 追加单事件
    pub async fn append(&self, ev: &liuma_session::EventEnvelope) -> Result<(), PersistenceError> {
        match self {
    ...
    /// 全量读取(读取方守卫生效)
    pub async fn load(&self) -> Result<Vec<liuma_session::EventEnvelope>, PersistenceError> {
        match self {
            Self::Jsonl(b) => b.load(),
