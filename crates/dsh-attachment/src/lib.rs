...
//! `user/message` 块数组里,零字节)。语义蓝本为源项目
//! packages/attachment(+ attachment-local):契约与本地落盘后端在
//! 这里合一——RS 仅本地单机一种后端,不立 contract/backend trait。
//!
//! 分层:类型词汇见 [`types`];落盘存储见 [`store`];请求期字节来源
//! 契约 [`AttachmentSource`] 由 dsh-llm 的方言翻译消费(随 [`store`]
//! 一并提供实现)。
