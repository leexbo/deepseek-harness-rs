...
//! RS 无部署时配置面——源预置 section 全文照抄,工具名两侧一致)。

use dsh_prompt::PromptSection;
use dsh_session::EventLog;

use crate::state::plan_state;

/// plan 模式约束段正文(照源 standard 预置 `planMode.section` 逐字;
/// 每请求渲染,未激活不注入)。
...
/// active-plan 段正文模板(RS 扩展段:批准后计划持续注入,压缩折叠后
/// 仍可循;源无此段——计划只活在 tool-call/result 历史)。
pub const ACTIVE_PLAN_SECTION_BODY: &str =
    "The user approved this plan. Implement it; track progress with the todo_write tool.";

/// 折叠日志得出的 header prompt 段组(喂 `dsh_prompt::AssembleContext`;
