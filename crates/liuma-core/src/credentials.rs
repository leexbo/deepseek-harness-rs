...
mod tests {
    ...
    #[test]
    fn explicit_wins() {
        let p = provider_named("liumawin", None, Some("from-settings"));
        assert_eq!(
    ...
    fn settings_key_beats_ref() {
        let p = provider_named(
            "liumaset",
            Some("env:LIUMA_SET_REF_VAR"),
            Some("from-settings"),
        );
        assert_eq!(
        ...
        let empty = provider_named("liumaset2", Some("env:LIUMA_SET_REF_VAR"), Some(""));
        unsafe { std::env::set_var("LIUMA_SET_REF_VAR", "from-ref") };
        assert_eq!(
            resolve_credential(None, &empty).as_deref(),
            Some("from-ref")
        );
        unsafe { std::env::remove_var("LIUMA_SET_REF_VAR") };
    }

    /// env: 引用可解析而变量缺席 = 缺席,不回落默认链;不可解析字面量跳过该级
    #[test]
    fn ref_semantics() {
        let var = default_env_name("liumaref");
        unsafe { std::env::set_var(&var, "from-default") };
        let dangling = provider_named("liumaref", Some("env:LIUMA_REF_MISSING_VAR"), None);
        assert_eq!(resolve_credential(None, &dangling), None);
        let unparseable = provider_named("liumaref", Some("keychain:liuma/x"), None);
        assert_eq!(
    ...
    fn default_env_fallback() {
        let p = provider_named("liumadef", None, None);
        let var = default_env_name("liumadef");
