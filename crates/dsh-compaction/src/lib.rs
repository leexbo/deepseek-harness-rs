...
//! `dsh-session`;本 crate 只做决策与指令构造,不做 IO、不调模型。
...
fn estimate_tokens(ty: &str, data: &serde_json::Value) -> u64 {
    dsh_session::message_from_event(ty, data)
...
pub fn select_range(events: &[EventEnvelope], retain: u64) -> Option<CompactRange> {
    ...
    for ev in &live {
        if dsh_session::message_from_event(&ev.r#type, &ev.data).is_some() {
    ...
    // dsh_session::derive_visible_messages),摘要前缀须含它——
...
mod tests {
    use super::*;
    use dsh_session::{EventLog, derive_visible_messages, message_from_event};
