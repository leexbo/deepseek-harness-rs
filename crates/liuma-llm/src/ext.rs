...
fn openai_style_body_error(body: &str) -> Option<liuma_agent_loop::TransportError> {
    ...
    match code {
        Some(code) => {
            ...
            Some(match status {
                ...
                None => liuma_agent_loop::TransportError::Other(format!(
                    "provider 错误 {code}: {message}"
                )),
            })
        }
        None => (!message.is_empty())
            .then(|| liuma_agent_loop::TransportError::Other(format!("provider 错误:{message}"))),
    }
}
...
impl ResponsesExt for GlmResponsesExt {
    ...
    fn body_error(&self, body: &str) -> Option<liuma_agent_loop::TransportError> {
        ...
        Some(match status {
            Some(status) => classify_status(status, None, body.to_string()),
            None => liuma_agent_loop::TransportError::Other(format!("provider 错误 {code}: {msg}")),
        })
    }
}
