...
use liuma_llm::adapters::ProviderAdapter;
use liuma_llm::anthropic::GenericAnthropicAdapter;
use liuma_llm::ext::{DeepSeekResponsesExt, OpenAiResponsesExt, StandardAnthropicExt};
use liuma_llm::http::{HttpTransport, ProviderConfig};
use liuma_llm::responses::GenericResponsesAdapter;
use liuma_llm::streaming::{StreamEvent, StreamMode};
...
fn anthropic_request_shape() {
    ...
    let body = adapter.build_request(
        ...
        &liuma_llm::attachments::NoAttachments,
    );
...
fn tool_message_with_images_wire_shape() {
    use liuma_llm::attachments::{AttachmentSource, OFFLOADED_IMAGE_TEXT};
    struct Fixed;
    ...
    let chat = liuma_llm::adapters::adapter_by_name("deepseek-chat").unwrap();
...
async fn anthropic_http_endpoint_and_auth() {
    ...
    .unwrap();
    use liuma_agent_loop::LlmTransport;
    ...
    assert_eq!(
        ...
        vec![
            ...
            LlmEventLike::done(),
            LlmEventLike(liuma_agent_loop::LlmEvent::Usage(
                serde_json::json!({ "ttftMs": 0 }),
            )),
        ]
        .into_iter()
        .map(Into::into)
        .collect::<Vec<liuma_agent_loop::LlmEvent>>()
    );
}
...
async fn deepseek_responses_http_endpoint_and_events() {
    ...
    let mut transport = HttpTransport::with_adapter(
        ...
        liuma_llm::adapter_by_name("deepseek-responses").unwrap(),
    )
    .unwrap();
    use liuma_agent_loop::LlmTransport;
    let events = transport
    ...
    assert_eq!(
        events,
        vec![
            LlmEventLike(liuma_agent_loop::LlmEvent::Reasoning(" hmm".into())),
            LlmEventLike::chunk("hi"),
            LlmEventLike(liuma_agent_loop::LlmEvent::Usage(json!({
                "input_tokens": 7, "output_tokens": 2
            }))),
            LlmEventLike::assistant("hi"),
            LlmEventLike::done(),
            LlmEventLike(liuma_agent_loop::LlmEvent::Usage(json!({ "ttftMs": 0 }),)),
        ]
        .into_iter()
        .map(Into::into)
        .collect::<Vec<liuma_agent_loop::LlmEvent>>()
...
async fn reasoning_only_stream_still_yields_ttft() {
    ...
    let tail = events.iter().rev().find_map(|e| match e {
        liuma_agent_loop::LlmEvent::Usage(u) if u.get("ttftMs").is_some() => Some(u.clone()),
        _ => None,
    });
    assert!(
        tail.is_some(),
...
struct LlmEventLike(liuma_agent_loop::LlmEvent);

impl LlmEventLike {
    fn chunk(s: &str) -> Self {
        Self(liuma_agent_loop::LlmEvent::Chunk(s.into()))
    }
    fn assistant(content: &str) -> Self {
        Self(liuma_agent_loop::LlmEvent::AssistantMessage(json!({
            "content": content, "tool_calls": []
        })))
    }
    fn done() -> Self {
        Self(liuma_agent_loop::LlmEvent::Done)
    }
}

impl From<LlmEventLike> for liuma_agent_loop::LlmEvent {
...
fn adapter_by_name_resolves_and_rejects() {
    assert!(liuma_llm::adapters::adapter_by_name("deepseek-responses").is_some());
    assert!(liuma_llm::adapters::adapter_by_name("openai-responses").is_some());
    assert!(liuma_llm::adapters::adapter_by_name("deepseek-chat").is_some());
    assert!(liuma_llm::adapters::adapter_by_name("openai-completions").is_some());
    assert!(liuma_llm::adapters::adapter_by_name("anthropic-messages").is_some());
    // 未知方言拒绝(fail-fast,不静默回退)
    assert!(liuma_llm::adapters::adapter_by_name("nope").is_none());
}
...
fn responses_user_image_wire_shape() {
    use liuma_llm::attachments::{AttachmentSource, OFFLOADED_IMAGE_TEXT};
    use liuma_llm::ext::GlmResponsesExt;
    struct Fixed;
    impl AttachmentSource for Fixed {
        fn image_bytes(&self, _id: &str) -> Option<Vec<u8>> {
            Some(vec![1, 2, 3])
...
async fn glm_responses_http_request_carries_input_image() {
    ...
    assert!(
        !json_body.contains(liuma_llm::attachments::OFFLOADED_IMAGE_TEXT),
